...
bool Cst820::check_communication()
{
    uint8_t chip_id;
    uint8_t soft_ver;
    if (!read_register(REG_CHIP_ID, &chip_id, 1)) return false;
    if (!read_register(REG_SOFT_VER, &soft_ver, 1)) return false;

    ESP_LOGI(TAG, "Chip ID: 0x%02x, Soft Ver: 0x%02x", chip_id, soft_ver);

    // Basic check: non-zero values usually indicate valid read
    return chip_id != 0 && soft_ver != 0;
}
...
bool Cst820::read()
{
    ...
    if (!read_register(REG_STATUS, buf, sizeof(buf))) {
        return false;
    }
    ...
    _pressed = _finger_num > 0 && (event_status == 0 || event_status == 2);
